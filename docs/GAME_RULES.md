...
11. **Stalemate redistribution (PLANNED):** a cut never cancels cards, so if (typically with 2 players) the active hands share no suit, every trick is a cut and the round can loop forever. When tricks resolve as cuts with no cancellation for too long (a stalemate), **top every active player's hand back up to 5 cards** — a player holding 2 cards receives 3, a player holding 3 receives 2. The top-up cards come from a reshuffled pool of cards that have left play (Part 1 discards + all cancelled-trick cards), so the game stays within one 52-card deck. The current leader then continues. See `docs/CALCULATIONS.md` §4.6 for the exact trigger, source, and algorithm. *(Not yet implemented.)*

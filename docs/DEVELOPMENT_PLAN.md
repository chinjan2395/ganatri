...
Last updated: 2026-06-20 (Phase 7e admin auth hardening — server: `adminSecret()` added to `config.ts` (reads `ADMIN_SECRET` at call time, not cached); `AdminAuthPayload.secret?: string` added to `protocol.ts`; `handlers.ts` ADMIN_AUTH handler now requires matching secret when `ADMIN_SECRET` env var is set (email-only backwards compat when unset); always responds `reason: 'not_authorized'` with no hint of which check failed; `ADMIN_SECRET` added to `.env.example`; 5 new tests in `admin.test.ts` (no-secret-backwards-compat, correct-secret, wrong-secret, missing-secret, wrong-email); `vitest.config.ts` env seeds `ADMIN_EMAILS=admin@test.com`; server tests 54→59.)

Last updated: 2026-06-20 (Phase 7e admin auth hardening — web: `AdminScreen` gains a `secret` state + password input ("Admin secret (leave blank if not required)") below the email field; `handleAuth` emits `{ email, secret }` instead of `{ email }` to `ADMIN_EVENTS.AUTH`; `not_authorized` error text updated to "Invalid credentials."; subtitle updated to "Enter your credentials to access game configuration."; submit button disabled condition unchanged (`!email.trim()`). Build green.)
...
| Admin secret enforcement (Phase 7e)                                   | ✅      | `adminSecret()` in `config.ts`; `secret?` on `AdminAuthPayload`; `handlers.ts` ADMIN_AUTH checks secret when `ADMIN_SECRET` env set; 5 new tests in `admin.test.ts` |
...
| Strengthen admin authentication | ✅ | `adminSecret()` added to `config.ts` (reads `ADMIN_SECRET` env at call time, not cached); `AdminAuthPayload.secret?: string` added to `protocol.ts`; `handlers.ts` `ADMIN_AUTH` now checks `payload.secret === adminSecret()` when `ADMIN_SECRET` is set (email-only auth still works when unset, for local dev backwards compat); always rejects with `reason: 'not_authorized'` (no hint which check failed); `ADMIN_SECRET` documented in `.env.example`; 5 new tests in `admin.test.ts` (no-secret backwards-compat, correct-secret, wrong-secret, missing-secret, wrong-email); `vitest.config.ts` seeds `ADMIN_EMAILS=admin@test.com`. Web `AdminScreen` collects optional `secret` input and emits `{ email, secret }` to `admin_auth`; `not_authorized` error shows "Invalid credentials." |

...
Last updated: 2026-06-25 (GameScreen layout polish to match redesign mockups: full-width desktop 3-column shell with new `LeftSidebar` (room info, players, chat); `TopBar` two-row mobile layout (logo image, timer/voice/current-turn status row); opponent ring positioning on mobile + horizontal strip on desktop; `GameTable` rounded-rect on mobile / oval on desktop; Part 2 suit-stacked hand rows; voice/history panels in `RightSidebar`. Build green.)

Last updated: 2026-06-25 (Game screen redesign — design token additions and component enhancements: (1) `theme.css` gains 7 new `:root` tokens: `--oval-border`, `--oval-glow`, `--card-stack-peek`, `--card-stack-hover-lift`, `--sidebar-mobile-w`, `--sidebar-desktop-w`, `--bottom-nav-h`. (2) `Hand.tsx` gains `StackRow` sub-component and stacked-rows rendering path (activated when `hand.length > 10`): splits hand into two rows with heavy overlap controlled by `--card-stack-peek`. (3) `Hand.css` gains `.hand--stacked`, `.hand__stack-row` and associated hover/selected lift rules. (4) `OpponentSeat.tsx` gains `showFan?: boolean` prop — adds `seat--with-fan` class and renders three `.seat__fan-card` divs as a decorative fan behind the avatar. (5) `OpponentSeat.css` gains `.seat--with-fan`, `.seat__fan`, `.seat__fan-card` (rotated card fan, blue striped background, gold border) and z-index rules to keep seat content above the fan.)

Last updated: 2026-06-25 (Game screen redesign components: created `GameTable.tsx` + `GameTable.css` (casino oval table wrapper with gold border, watermark, dealer chip, turn status badge); `MobileLeftPanel.tsx`, `MobileRightPanel.tsx`, `MobilePanel.css` (deck counter + auto-arrange toggle + sort button; cuts counter + claim-capture button). All 5 files new — no existing files modified. Build green, zero TS errors.)

Last updated: 2026-06-25 (Game screen redesign components: `TopBar`, `BottomNav`, `RightSidebar` new components + CSS added to `packages/web/src/components/`. Build green, zero TS errors.)
...
All 452 tests passing (153 engine + 108 server + 191 db).
...
| `GameTable` — casino oval table wrapper (gold border, watermark, dealer chip, turn status) | ✅ | `src/components/GameTable.tsx` + `GameTable.css`; accepts `children`, `statusText`, `isYourTurn`, `flashNode` |
| `MobileLeftPanel` — deck counter, auto-arrange toggle, sort button         | ✅      | `src/components/MobileLeftPanel.tsx` + `MobilePanel.css`                  |
| `MobileRightPanel` — cuts counter, claim-capture button (part1 only)       | ✅      | `src/components/MobileRightPanel.tsx` + `MobilePanel.css`                 |
| `TopBar` — game HUD header (hamburger, phase pill, logo, leave button)     | ✅      | `src/components/TopBar.tsx` + `TopBar.css`; mobile logo + hamburger, desktop hides both; timer/voice slots |
| `BottomNav` — mobile-only 4-tab nav with slide-up sheet panels             | ✅      | `src/components/BottomNav.tsx` + `BottomNav.css`; chat/score/history/more tabs; hidden ≥768px |
| `RightSidebar` — desktop-only right panel (turn, deck, actions, cuts, history) | ✅  | `src/components/RightSidebar.tsx` + `RightSidebar.css`; hidden on mobile, flex on ≥768px |

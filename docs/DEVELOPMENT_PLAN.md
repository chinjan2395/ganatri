...
| Privacy-respecting collection | ✅ | Anonymous IDs only (`distinct_id: 'server'`); no display names, emails, or room codes in event props — only counts, durations, and booleans. Documented in `analytics.ts` file header. |

...
All 332 tests passing (153 engine + 50 server + 129 db).
...
| Phase 6 — Persistence/DB     | 🟡 6a complete (pg Pool + regenerated migration); 6b durable `GamePersistence` layer built & fully tested (129 db tests, pglite); 6d live write-through wired into server (games/events/players) ✅ + 6e stats increments ✅ + 6c guest→registered upgrade flow ✅. Restart-rehydration (`loadActiveGames`) deferred; server `MemoryStore` refactor + accounts/analytics UI (6f–6j) remain. |

...
| DS-R24: `GameScreen` desktop layout — new `GameTable` DS component + casino-table chrome matching `RoomScreen` | ✅ | New DS component `packages/ds/src/components/GameTable/` (`GameTable.tsx`+`.css`+`index.ts`, exported from `packages/ds/src/index.ts`): supports 2-4 seats (`GameTableSeatData[]`, last seat always bottom-anchored "you") + a `children` center play-area slot; CSS adapted from `OvalTable.css`/`SeatSlot.css` (oval/rail/felt/rail-light gold styling), oval sized larger (560-700px) to fit a live board, seat position presets keyed by `game-table__seat--{count}-{index}`. `GameScreen.tsx`: added `isDesktop` branch (before the existing unbranched `!view`/`GAME_OVER` early returns) rendering `game__hud-desktop` (phase badge + `TurnTimer` + Leave), a 3-column `game__desktop-body` grid (left `DsCard`+`DsTitleBlock` game-info panel / center `GameTable` wrapping `Part1Board`/`Part2Board`+stock+flash+phase-transition + `game__hand-section-desktop` / right isolated `GameVoicePanel` subcomponent wrapping `VoiceChatPanel`, mirrors the `PlayerWrap` speaking-context isolation pattern); reused `CapturedPile`/`CutAnimation`/action-bar JSX verbatim. Added `toBottomAnchoredSeats` helper (new, does not touch `orderedPlayers`) and a local `getInitials` helper (copied from `RoomScreen`). `--card-table-w` cap raised to 92px on desktop (was 64px both). Mobile path (`isDesktop === false`) is the pre-existing final `return` — byte-for-byte unchanged (verified via `git diff`, only additions, zero deletions in the JSX below the new branch). `GameScreen.css`: appended new `.game--desktop`/`.game__hud-desktop`/`.game__desktop-body`/`.game__left-col`/`.game__center-col`/`.game__right-col`/`.game__info-card`/`.game__hand-section-desktop` rules at end of file (pure append, 0 existing lines touched, verified via diff). TS strict clean (`tsc --noEmit` 0 errors across all workspaces), ESLint clean, all 142 engine+server tests still pass (web/ds have no test runner configured). (done 2026-07-01) |
| DS-R25: `GameScreen` desktop Round 2 — match reference screenshot (felt/particles, bigger table, ScoreCard, Actions card replaces bottom sheet) | ✅ | New DS component `packages/ds/src/components/ScoreCard/` (`ScoreCard.tsx`+`.css`+`index.ts`+`.stories.tsx`, exported from `packages/ds/src/index.ts` near `DsStatCard`): `ScoreCardEntry[]` rows (rank/name/score/optional pulsing turn-dot/leader accent), gold-uppercase heading matching `DetailsSidebar`. `GameScreen.tsx` desktop branch (only): root now renders `.game__particles` (8 `<span>` dots, RoomScreen's float-up pattern under new class names) as a sibling of the felt backdrop; header kept same content, CSS-only restyle to `HeaderDesktop`'s transparent/gold-flourish language; left column now stacks a new "GAME INFO" row-grid block (Room ID + Round, new `.game__info-row` classes mirroring `DetailsSidebar`, not reusing its classes) + the new `ScoreCard` (fed by a new `scoreEntries` `useMemo` off `playerSeatData`: score = captureCount in Part 1 / safeOrder in Part 2, sorted desc, leader flagged only when the top score is unique and > 0); right column now stacks CURRENT TURN (`DsCard`, avatar+name of the seat where `isTurn`) → DECK (`DsCard`, `view.stockCount`, Part 1 only) → ACTIONS (`DsCard`, persistent — same confirm-card/choose-capture markup and handlers as before, now always visible instead of an animated bottom sheet; idle state shows `handState.hint` or a fallback) → `GameVoicePanel` (unchanged); removed the old in-table absolute `table-stock` pile and the old `<AnimatePresence>` bottom-sheet block from the desktop branch only (both fully preserved, byte-for-byte, in the separate mobile `return` further down the file). `GameTable.css` oval breakpoints enlarged ~20% (680/780/860 × 390/430/470, was 560/640/700 × 320/350/380) — pure value tuning, no structural changes. Judgment calls: dropped the in-table stock pile entirely on desktop (the new DECK card fully replaces it, no duplication); leader tie-handling — only the sorted top entry gets `isLeader`, and only if its score is nonzero and strictly greater than the next entry's (simple, no multi-way-tie leader). TS strict clean (`tsc --noEmit` 0 errors), ESLint clean, all 518 tests still pass (223 db + 153 engine + 142 server — unchanged, this is UI-only). Mobile `return` block and all pre-existing CSS above the desktop section verified byte-identical via `git diff` (single contiguous hunk per file, both starting exactly at the desktop-only boundary). **Post-review fixes:** (1) fixed a Part 2 scoreboard bug where `safeOrder` (1-indexed, lower = better/winner) was sorted descending like a normal score, inverting the leader highlight — now sorts ascending by rank for Part 2 (still-playing players sort last) while Part 1 still sorts descending by capture count; (2) restored the live `latestMatchScoring` badges (dropped entirely on desktop in the first pass) inside the GAME INFO card, below the Room ID/Round rows; (3) swapped the raw `<h3 className="game__info-heading">` for `<DsSectionHeading level={3} className="game__info-heading">` to match this phase's DS-consolidation goal. Re-verified: `tsc --noEmit` and `eslint src` clean, all 142 server + engine/db tests still pass, mobile diff still isolated to the same 3 shared lines. **Round 3 (visual polish, same day):** (1) moved player seats off the `GameTable` oval rim entirely (`GameTable seats={[]}` now, removed the now-unused `toBottomAnchoredSeats` helper) into a new horizontal row above the table (`.game__players.game__players--desktop`, reusing mobile's exact dark-indigo/gold-diamond-lattice/shimmer background so desktop matches the reference screenshot's top player row instead of scattering seats around the oval); (2) fixed a DS-consistency gap — `ScoreCard` had its own bespoke card shell (copied `DetailsSidebar`'s look, 10px radius) instead of the shared `DsCard` primitive that CURRENT TURN/DECK/ACTIONS already used, making it look visually inconsistent/"dull"; `ScoreCard` now composes `<DsCard title={title}>` internally (kept its own distinct row-list content/leader-highlight design inside), and `ScoreCard.css` dropped all the now-redundant shell rules (background/border/radius/shadow/heading); (3) same fix applied to the left column's "GAME INFO" block — swapped the raw `<div className="game__info-card">` for `<DsCard className="game__info-card" title="GAME INFO">`, and removed the now-dead `.game__info-heading`/duplicate shell CSS from `GameScreen.css`. Re-verified: `tsc --noEmit` clean (web + ds), `eslint src` clean, all 518 tests still pass (223 db + 153 engine + 142 server), mobile diff still isolated to the same 3 shared lines (import statement + card-width cap formula + effect deps). **Round 4 (PLAYERS + CHAT placeholder, same day):** added a new "PLAYERS (N/4)" `DsCard` in the left column between GAME INFO and ScoreCard, listing every seat (`playerSeatData`) with a small avatar circle (image or initials), display name (+ "(You)" suffix, + host crown `♛` when `room?.hostId === seat.pid`), and an online/offline status dot (green pulse vs. dim grey, driven by `seat.isDisconnected`) — new `.game__players-card/-list/-row/-avatar/-name/-crown/-status(--offline)` classes. Added a "CHAT" `DsCard` below ScoreCard using the existing `DsEmptyState` primitive ("Chat is coming soon") since chat isn't implemented yet — `.game__chat-card` is `flex:1` so it fills remaining left-column height, matching the reference screenshot's layout. Re-verified: `tsc --noEmit` clean (web + ds), `eslint src` clean, all 518 tests still pass, mobile diff still isolated to the same 3 shared lines. **Round 5 (merge PLAYERS + score, richer visual design, same day):** removed the separate `ScoreCard` usage from `GameScreen.tsx` (the DS `ScoreCard` component itself is left in `packages/ds` as a still-valid, tested, reusable primitive — just no longer consumed by this screen) and folded live score into the PLAYERS card itself: `scoreEntries` renamed/extended to `playersPanelData` (same Part 1 capture-count-descending / Part 2 safe-rank-ascending sort + leader logic as before, now carrying the full seat identity fields too instead of a stripped-down `ScoreCardEntry` shape). Redesigned the PLAYERS row from a flat, single-line list (small flat avatar circle, plain text, tiny dot) into a richer per-row layout: rank number (or a 👑 medal on the leader), a gradient hue-tinted avatar circle with a glossy top highlight (new `hueFor` helper in `GameScreen.tsx`, mirroring `OpponentSeat`'s existing per-player-color technique) plus a small pulsing blue turn-indicator badge on the avatar's corner when it's that player's turn, a two-line name+online/offline status block, a host crown, and a right-aligned gold score value — with the whole row getting a subtle colored glow/border treatment when it's the current turn (blue) and/or the leader (gold, stronger if both). New CSS: `.game__players-row--turn/--leader/--offline`, `.game__players-rank`, `.game__players-avatar` (gradient/glossy, replacing the old flat version), `.game__players-turn-dot` (+ pulse keyframes), `.game__players-info`, `.game__players-online(--offline)`, `.game__players-score`; removed the old flat `.game__players-status(--offline)` rules (superseded by the turn-dot + online-text approach) and the now-unused `.game__score-card` rule. `DsCard`'s existing `subtitle` prop used to show "Captures"/"Safe rank" under the PLAYERS heading instead of a separate CAPTURES/SAFE RANK card title. Re-verified: `tsc --noEmit` clean (web + ds), `eslint src` clean, all 518 tests still pass, mobile diff still isolated to the same 3 shared lines. **Post-review fix (Round 5):** the PLAYERS score column showed a literal `0` for still-playing players during Part 2 ("Safe rank" 0 doesn't exist — `safeOrder` is 1-indexed, so 0 read as a fake rank rather than "still playing"); render an em dash (`–`) instead of `0` when `view.phase === 'PART_2' && seat.safeOrder === undefined`. The underlying sort/leader logic was already correct (still-playing players already sorted last and never got the leader flag) — this was a display-only fix. Re-verified: `tsc --noEmit` clean, `eslint src` clean, mobile diff unchanged. **Round 6 (right-column reorder + toned-down highlights, same day):** (1) removed the CURRENT TURN card entirely (right column now: DECK → `GameVoicePanel` → ACTIONS) and dropped its now-dead `.game__turn-card-body/-avatar/-name` CSS + the unused `turnSeat` variable; (2) moved ACTIONS to the LAST position in the right column so it's vertically adjacent to the hand tray in the center column, per user feedback that it was "very far from card selection at bottom"; (3) toned down the PLAYERS card's current-turn row highlight — `.game__players-row--turn` dropped its radial-gradient wash and blue glow box-shadow, now just a faint background tint + subtle border tint (leader highlight untouched, still the stronger gold glow); (4) removed the "You" gold ring/glow around the local player's avatar in the desktop top player row (`.game__players--desktop .seat--you .seat__avatar`, resets `box-shadow` back to the normal non-you avatar ring) — this only targets the desktop-scoped `.game__players--desktop` wrapper class, so mobile's `.seat--you` styling (used on phones, shared `OpponentSeat.css`) is completely unaffected; (5) shrank the CHAT placeholder card from `flex:1` (filled all remaining left-column height) to a small fixed-size card (`flex-shrink:0`, tighter padding/header-margin/font-size via `!important` overrides on `DsCard`/`DsEmptyState` internals — no prop/API exists on either DS component to control this sizing, so `!important` is the pragmatic screen-level override; there is no direct precedent for this specific use in `RoomScreen.css`, whose 3 `!important` usages are unrelated — a `.room__seat-circle--speaking` voice-activity ring animation, not DsCard/DsEmptyState overrides). Re-verified: `tsc --noEmit` clean (web + ds), `eslint src` clean, all 518 tests still pass, mobile diff still isolated to the same 3 shared lines. **Post-review fix (Round 6):** the "You"-ring-removal selector (`.game__players--desktop .seat--you .seat__avatar`) tied in CSS specificity (0,3,0) with `OpponentSeat.css`'s `.seat--you.seat--turn .seat__avatar` gold pulse rule, and won the tie purely because `GameScreen.css` is imported after `OpponentSeat.css` — silently cancelling the gold turn-pulse glow on your own avatar whenever it was your turn on desktop, on top of the CURRENT TURN card's removal this same round leaving no other avatar-level turn cue. Fixed by scoping the override to `.game__players--desktop .seat--you:not(.seat--turn) .seat__avatar`, so it no longer matches (and no longer competes) when `.seat--turn` is present — `OpponentSeat.css`'s turn-pulse rule now wins outright with no competing rule. Re-verified: `tsc --noEmit` clean (web + ds), mobile diff unchanged. (done 2026-07-01) |

...
| DS-R23: LobbyScreen `CreateJoinPanel` headings + sidebar lists | ✅ | `CreateJoinPanel` `<div className="lobby__cj-heading">` × 2 → `DsTitleBlock size="sm"`; `<div className="lobby__cj-sub">` × 2 → `DsBodyText tone="muted"`. Sidebar leaderboard `<ul className="sidebar__player-list"><li>` → `<div>` + `DsRankRow compact` (all fields from `LeaderboardEntryView`). Sidebar stats `<ul className="sidebar__stat-list"><li>` → `<div>` + `DsListRow` (title=label, trailing=value span). `lobby__blocked-list` `<ul><li>` → `<div>` + `DsListRow` (title=displayName, subtitle="Blocked", trailing=unblock `DsButton`). Added `DsRankRow` + `DsListRow` to DS import. Removed CSS blocks: `.lobby__cj-heading`, `.lobby__cj-sub`, `.sidebar__player-row`, `.sidebar__rank`, `.sidebar__player-avatar`, `.sidebar__player-name`, `.sidebar__player-wins`, `.sidebar__stat-row`, `.sidebar__stat-label`, `.lobby__blocked-list`, `.lobby__blocked-row`, `.lobby__blocked-name`. Kept `.sidebar__player-list`/`.sidebar__stat-list` wrappers (margin only) and `.sidebar__stat-value` (used as trailing content). Mobile `rp__rows` co-player rows deferred. Build + ESLint green; all 470 tests pass. (done 2026-06-30) |

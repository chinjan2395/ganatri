...
```
/ganatri <text>      → Worker → GitHub repository_dispatch (slack-add-todo)
                              → .github/workflows/slack-add-todo.yml (Claude formats + commits)
                              → posts "✅ Queued  [▶️ Run now]" back to the channel
click "▶️ Run now"   → Worker → GitHub workflow_dispatch (nightly.yml) → nightly run
```
...
**a) Slash command** — Features → **Slash Commands** → **Create New Command**:
- **Command:** `/ganatri`
